                ...
                for apm in [Time.AM, Time.PM]:
                    ttmodel.add_constraint(
                        ttmodel.sum(ttmodel.TT[sl, c]
                                    ...
                                    & ttmodel.wdb.compatible_slots[c]),
                        '<=',
                        2, constraint_type="Pas plus d'un TD par demie-journee_%s_%s_%s_%s_%g" % (
                        m, g, d, apm, ttmodel.constraint_nb))
                ...
                for d in days_filter(ttmodel.wdb.days, week=week):
                    for apm in [Time.AM, Time.PM]:
                        ttmodel.add_constraint(
                            ...
                            '<=',
                            4 * 60,
                            constraint_type="Pas plus de 4 heures par demie_journee_%s_%s_%s" % (t.username, d, apm))
                ...
                for i in range(len(C1)):
                    ...
                    for sl in ttmodel.wdb.compatible_slots[c1]:
                        ttmodel.add_constraint(ttmodel.TT[sl, c1] - ttmodel.TT[sl, c2], '==', 0,
                                               constraint_type="Simul_fake_%s_%s_%s_%s_%s" % (
                                               G1[nb].name, G2[nb].name, m, sl,
            ...
            for d in ttmodel.wdb.days:
                ttmodel.add_constraint(
                    ttmodel.sum(ttmodel.TT[sl, c]
                                ...
                                & ttmodel.wdb.compatible_slots[c]),
                    '<=',
                    2, constraint_type="Pas plus de seance du meme module par jour_%s_%s_%s_%g" % (
                    m, bg, d, ttmodel.constraint_nb))
        ...
        if len(Cg1) == len(Cg2):
            for d in ttmodel.wdb.days:
                ttmodel.add_constraint(ttmodel.sum(ttmodel.TT[sl, c]
                                       ...
                                       '==',
                                       0,
                                       constraint_type="G1 et G2 ont cours le meme jour %s %s _%g" % (
                                       m, d, ttmodel.constraint_nb))
