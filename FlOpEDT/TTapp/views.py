...
def available_work_copies(req, department, year, week):
    ...
    '''
    copies = list(ScheduledCourse.objects.filter(course__week__year=year, course__week__nb=week, course__type__department__abbrev=department).distinct('work_copy').values_list('work_copy'))
