...
def get_course_list():
    return ScheduledCourse.objects.filter(work_copy=0).order_by('course__an', 'course__week', 'slot__day_id', 'slot__hour')


def create_event(c):
    begin = datetime.combine(Week(c.course.an, c.course.week).day(c.slot.day_id-1),
                             datetime.min.time()) \
